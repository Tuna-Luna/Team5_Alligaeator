...
# sp3
# b4
image_sources = ["./food/b4.jpg", "./food/ct2.jpg", "./food/sp3.jpg", "./food/cb2.jpg"]
image_idx = 0
app = FastAPI()
...
def detect():
    ...
    try:
        source_path = image_sources[image_idx]
        image_idx = (image_idx+1)%len(image_sources)
        t = datetime.now().strftime("%H%M%S")
        food_item = detect_objects(
            weights='best2.pt',
            #source = source_path,
            #source=f'./food/ct2.jpg',
            source = 0,
            device='cpu',
            temp_img_save_path= f'./captures/captured_{t}.jpg'
        )
        
        print("class id : ", food_item[0][0]['class_id'], "food detected : ",  food_item[0][0]['name'])
